...
import MenuData from './custom-menu.json';

function CreateChild(id, iscurrentState, setIscurrentState, updateIconSidebar, subItems) {
  let items = [];
  subItems?.map((menu) => {
    let item_id = id + '#' + menu.id;
    let permissions = [];
    if (menu.permission !== 'EMPTY') {
      try {
        permissions = UserPermissions[menu?.permission?.split('.')[0]][menu?.permission?.split('.')[1]];
      } catch (error) {
        //
      }
    }
    let json = {
      ...
      parentId: id,
      stateVariables: iscurrentState,
      click: function (e) {
        e.preventDefault();
        setIscurrentState(iscurrentState.includes(menu.id) ? id : item_id);
        updateIconSidebar(e);
      },
    };
    if (menu.children !== undefined && menu.children.length > 0) {
      json['isChildItem'] = true;
      json['childItems'] = CreateChild(item_id, iscurrentState, setIscurrentState, updateIconSidebar, menu.children);
    }
    items.push(json);
  });
  return items;
...
let CustomMenu = ({ iscurrentState, setIscurrentState, updateIconSidebar, section, customMenu }) => {
  let finalMenu = [];
  customMenu.map((menu) => {
    let permissions = [];
    if (menu.permission !== 'EMPTY') {
    ...
    if (menu.type === 'page') {
      let json = {
        ...
        new: menu.new ?? false,
        tag: menu.tag ?? '',
        tagClass: menu.tagClass ?? '',
        permissions: permissions,
        stateVariables: iscurrentState,
        click: function (e) {
          e.preventDefault();
          setIscurrentState(iscurrentState.includes(menu.id) ? '' : menu.id);
          updateIconSidebar(e);
        },
      };
      if (menu.children !== undefined && menu.children.length > 0) {
        json['subItems'] = CreateChild(menu.id, iscurrentState, setIscurrentState, updateIconSidebar, menu.children);
      }
      finalMenu.push(json);
    }
...
let ProductMenu = ({ isApps, setIsApps, setIscurrentState, updateIconSidebar, isLevel1, setIsLevel1, isLevel2, setIsLevel2 }) => {
  return [
    {
      label: 'Menu',
      isHeader: true,
      ...
      subItems: [
        {
          ...
          childItems: [
            { id: 1, icon: 'bx bx-file', label: 'Level 2.1', link: '/#' },
            {
              icon: 'bx bx-file',
              id: 'level2.2',
              label: 'Level 2.2',
              ...
              childItems: [
                { id: 1, icon: 'bx bx-file', label: 'Level 3.1', link: '/#' },
                { id: 2, icon: 'bx bx-file', label: 'Level 3.2', link: '/#' },
              ],
            },
          ],
        },
        { id: 'addons', icon: 'bx bx-add-to-queue', label: 'Add-Ons', link: '#', parentId: 'workspace' },
      ],
    },
  ];
};
...
let WappFlowMenu = () => {
  return [
    ...
    },
    {
      ...
      permissions: UserPermissions.WAPPFLOW.FLOWS,
    },
    // {
    //   label: 'Template',
    //   isHeader: true,
