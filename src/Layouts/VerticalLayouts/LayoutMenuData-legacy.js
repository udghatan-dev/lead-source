...
import { userAutomationMenu, userBotMenu, adminAutomationMenu, adminBotMenu, userAdManagerMenu } from './menuData';
import md5 from 'md5';
import { useProduct } from '../../Components/Hooks/ProductHooks';
import { AutomationMenu, BotMenu, CRMMenu, ProductMenu, VirtualNumberMenu, WABAMenu, WorkspaceMenu } from './allMenuData';
import { decrypt } from '../../security';

const allowedEmail = ['akshayshardaohline@gmail.com', 'hq@salesrook.com'];
...
const Navdata = () => {
  ...
  const {
    ...
    //themeSetting: state.Panel.themeSetting,
  ...
  if (userRNP !== null) {
    ...
    // if (userType === 'user' || userType === 'agency' || userType === 'team_member') {
    //   if (pathName === '/products' || pathName === '/products/' || pathName === '/' || pathName.includes('/products/e/')) {
    //     var menuItems = userProductMenu(userRNP).map((menu) => {
    //       return {
    //         ...menu,
    ...
    //           setIscurrentState(menu.label);
    //           updateIconSidebar(e);
    //         },
    //       };
    //     });
    //   }
    //   if (pathName.includes('/products/waba') || pathName === '/website/settings' || pathName === '/website/settings/') {
    //     var menuItems = userWABAMenu(userRNP).map((menu) => {
    //       return {
    //         ...menu,
    //         stateVariables: isDashboard,
    //         click: function (e) {
    ...
    //     var menuItems = userCRMMenu(userRNP).map((menu) => {
    //       return {
    //         ...menu,
    //         stateVariables: isDashboard,
    //         click: function (e) {
    ...
    //     });
    //   }
    //   if (pathName.includes('/products/aiwriter')) {
    //     var menuItems = userAutoBloggingMenu(userRNP).map((menu) => {
    //       return {
    //         ...menu,
    //         stateVariables: isDashboard,
    //         click: function (e) {
    ...
    //     var menuItems = userWorkspaceMenu(userRNP).map((menu) => {
    //       return {
    //         ...menu,
    //         stateVariables: isDashboard,
    //         click: function (e) {
    ...
    //   if (pathName === '/products' || pathName === '/products/' || pathName === '/' || pathName.includes('/products/e/')) {
    //     var menuItems = developerProductMenu.map((menu) => {
    //       return {
    //         ...menu,
    //         stateVariables: isDashboard,
    //         click: function (e) {
    ...
    //   }
    // } else {
    //   var panel = userRNP.account.white_label_panel;
    //   if ((userType === 'white_label_agency' && panel === window.location.hostname) || userType === 'admin') {
    //     if (pathName === '/products' || pathName === '/products/' || pathName === '/' || pathName.includes('/products/e/')) {
    //       var menuItems = whiteLabelProductMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    //           click: function (e) {
    ...
    //     }
    //     if (pathName.includes('/products/waba') || pathName === '/website/settings/' || pathName === '/website/settings') {
    //       var menuItems = whiteLabelWABAMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    //           click: function (e) {
    ...
    //     if (pathName.includes('ticket-manager')) {
    //       var menuItems = whiteLabelProductMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    ...
    //     if (pathName.includes('/products/crm')) {
    //       var menuItems = whiteLabelCRMMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    //           click: function (e) {
    ...
    //     if (pathName.includes('/products/aiwriter')) {
    //       var menuItems = userAutoBloggingMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    //           click: function (e) {
    ...
    //   } else {
    //     if (pathName === '/products' || pathName === '/products/' || pathName === '/' || pathName.includes('/products/e/')) {
    //       var menuItems = agencyProductMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    //           click: function (e) {
    ...
    //     }
    //     if (pathName.includes('/products/waba') || pathName === '/website/settings' || pathName === '/website/settings/') {
    //       var menuItems = agencyWABAMenu.map((menu) => {
    //         return {
    //           ...menu,
    //           stateVariables: isDashboard,
    //           click: function (e) {
    ...
    //   if (userType === 'white_label_agency' || userType === 'admin') {
    //     if (pathName.includes('/workspace')) {
    //       var menuItems = whiteLabelWorkspaceMenu.filter((menu) => {
    //         if (menu.id === 'panel-user') {
    //           if (userType === 'white_label_agency') {
    //             if (panel === window.location.hostname) {
    //               return {
    //                 ...menu,
    //                 stateVariables: isDashboard,
    //                 click: function (e) {
    ...
    if (pathName === '/products' || pathName === '/products/' || pathName === '/' || pathName.includes('/products/e/')) {
      var menuItems = ProductMenu().filter((menu) => {
    ...
    if (pathName.includes('/products/waba') || pathName === '/website/settings' || pathName === '/website/settings/') {
      var menuItems = WABAMenu().filter((menu) => {
        if (menu.permissions.length > 0) {
          let available = false;
          access.map((perm) => {
            if (menu.permissions.indexOf(perm) !== -1) {
    ...
    if (pathName.includes('/products/virtual_number')) {
      var menuItems = VirtualNumberMenu().filter((menu) => {
        if (menu.permissions.length > 0) {
          let available = false;
          access.map((perm) => {
            if (menu.permissions.indexOf(perm) !== -1) {
    ...
    if (pathName.includes('/products/crm')) {
      var menuItems = CRMMenu().filter((menu) => {
        if (menu.permissions.length > 0) {
          let available = false;
          access.map((perm) => {
            if (menu.permissions.indexOf(perm) !== -1) {
    ...
    if (pathName.includes('/workspace')) {
      var menuItems = WorkspaceMenu().filter((menu) => {
        if (menu.permissions.length > 0) {
          let available = false;
          access.map((perm) => {
            if (menu.permissions.indexOf(perm) !== -1) {
    ...
    if (pathName === '/products' || pathName === '/products/' || pathName === '/') {
      if (pageSettings !== null && pageSettings.filter((p) => p.type === 'menu').length > 0) {
        pageSettings
          ...
          .map((item) => {
            menuItems.push({
              ...
              label: item.label,
              icon: item.icon,
              link: item.iframe ? '/products/e/' + md5(item.label) : '#',
              isCustom: true,
              click: function (e) {
                if (!item.iframe) {
    ...
    if (pathName.includes('/products/automation')) {
      var menuItems = AutomationMenu().filter((menu) => {
        if (menu.permissions.length > 0) {
          let available = false;
          access.map((perm) => {
            if (menu.permissions.indexOf(perm) !== -1) {
    ...
    if (pathName.includes('/products/bot')) {
      var menuItems = BotMenu().filter((menu) => {
        if (menu.permissions.length > 0) {
          let available = false;
          access.map((perm) => {
            if (menu.permissions.indexOf(perm) !== -1) {
    ...
    if (pathName.includes('/products/admanager')) {
      if (userType === 'admin' || userType === 'developer') {
        var menuItems = userAdManagerMenu.map((menu) => {
          return {
            ...menu,
            stateVariables: isDashboard,
            click: function (e) {
