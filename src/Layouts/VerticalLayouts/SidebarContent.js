            ...
            {item['isHeader'] ? (
              ...
              <li className='nav-item'>
                <Link onClick={item.click} className='nav-link menu-link' to={item.link ? item.link : '/#'} data-bs-toggle='collapse'>
                  {item?.iconType === 'component' ? item.icon : <i className={item.icon}></i>}
                  <span data-key='t-apps'>{props.t(item.label)}</span>
                </Link>
                <Collapse className='menu-dropdown' isOpen={item.stateVariables} id='sidebarApps'>
                  <ul className='nav nav-sm flex-column test' style={{ listStyleType: 'none' }}>
                    {/* subItms  */}
                    {item.subItems &&
                      (item.subItems || []).map((subItem, key) => (
                        <React.Fragment key={key}>
                          {!subItem.isChildItem ? (
                            <li className='nav-item custom-subitem-nav menu-transform'>
                              <Link to={subItem.link ? subItem.link : '/#'} className='nav-link'>
                                {subItem?.iconType === 'component' ? subItem.icon : ''}
