...
import { connectContactForm7 } from '../../../helpers/backend_helper';
...
const ContactForm7Modal = ({ isOpen, toggle, onSuccess }) => {
  ...
  const handleCreate = async () => {
    ...
    setError('');
    try {
      const res = await connectContactForm7({ accountName: toSlug(name), name });
      setResult(res.data || res);
      onSuccess?.();
    } catch (err) {
      setError(err?.msg || err?.response?.data?.msg || 'Failed to create Contact Form 7 connection.');
    } finally {
      setCreating(false);
    }
  };
    ...
    <Modal isOpen={isOpen} toggle={handleClose} size='md' centered>
      <ModalHeader toggle={handleClose}>
        <div className='d-flex align-items-center gap-2'>
          <img src={`${ICON_PATH}/contact-form-7-icon.png`} alt="Contact Form 7" style={{ width: 32, height: 32, objectFit: 'contain' }} />
...
export default ContactForm7Modal;
