...
import { connectPhoneContact } from '../../../helpers/backend_helper';

const toSlug = (str) => str.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9_-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '');

const PhoneContactModal = ({ isOpen, toggle, onSuccess }) => {
  ...
  const handleCreate = async () => {
    ...
    setError('');
    try {
      const res = await connectPhoneContact({ accountName: toSlug(name), name });
