...
import { connectOcrApp } from '../../../helpers/backend_helper';

const toSlug = (str) => str.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9_-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '');

const OcrAppModal = ({ isOpen, toggle, onSuccess }) => {
  ...
  const handleCreate = async () => {
    ...
    setError('');
    try {
      const res = await connectOcrApp({ accountName: toSlug(name), name });
      setResult(res.data || res);
      onSuccess?.();
    } catch (err) {
      setError(err?.msg || err?.response?.data?.msg || 'Failed to create phone contact connection.');
    } finally {
      setCreating(false);
    }
  };
    ...
    <Modal isOpen={isOpen} toggle={handleClose} size='md' centered>
      <ModalHeader toggle={handleClose}>
        <div className='d-flex align-items-center gap-2'>
          ...
          <span>Create OCR App Connection</span>
        </div>
      </ModalHeader>
      <ModalBody>
        {error && (
        ...
        {result ? (
          <>
            <Alert color='success' className='mb-3' style={{ fontSize: '0.85rem' }}>
              OCR App connection created successfully!
            </Alert>
