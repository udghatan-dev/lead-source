              ...
              if (fileExist.length > 0) {
                ...
                Axios.get(url)
                  .then((response) => {
                    ...
                    } else if (version_latest !== version_local) {
                      localStorage.setItem('_v', version);
                      resolve({ status: true, load: true, data: data.Contents });
                    } else {
                      if (localStorage.getItem('_gs_key') && localStorage.getItem('_w')) {
    ...
    try {
      ...
      if (fileExist.length > 0) {
        ...
        Axios.get(url).then((response) => {
          ...
          var apiUrlSettings = {
            waba: product.WABA_API ?? 'https://waba.1automations.com',
            inbox: product.INBOX_API ?? 'https://crmapi.1automations.com',
            livechat: product.LIVE_CHAT_PANEL ?? 'https://inbox.1automations.com',
            automation: product.AUTOMATION ?? 'https://webhooks.1automations.com',
